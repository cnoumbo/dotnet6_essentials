...
namespace Linq
{
    ...
    internal class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            // - Filtrer
            Console.WriteLine("=== List of country with area less than 1000km2");
            var less1000km2 = listePays.Where(p => p.Superficie < 1000);
            foreach (var elt in less1000km2)
            {
                Console.WriteLine(elt);
            }

            var top5population = villes
                .OrderByDescending(v => v.Population)
                .Select(v => v.Pays)
                .Distinct()
                .Take(5);
            Console.WriteLine("=== Top 5 country with most populate city");
            foreach (var elt in top5population)
            {
                Console.WriteLine(elt);
            }

            // - Agréger
            var countryAreaAverage = listePays.Average(p => p.Superficie);
            Console.WriteLine($"Area Average : {countryAreaAverage}");

            var top5List = top5population.Aggregate(
                new StringBuilder(),
                (sb, p) => sb.Append(sb.Length == 0 ? p.Nom : $", {p.Nom}")
            );
            Console.WriteLine(top5List);

            // - Convertir
            var names = listePays.Select(p => p.Nom).OrderBy(n => n).ToList();
            var orderedCountry = listePays.OrderBy(p => p.Nom).ToList();
            
            Console.WriteLine($"'France' index {names.BinarySearch("France")}");
            // Perform binary search with IComparer
            // Console.WriteLine($"Position de l'Italie : {orderedCountry.BinarySearch()}");
        }
    }
}